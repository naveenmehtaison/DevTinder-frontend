...
const Feed = () => {
  ...
  const handleLeftClick = async () => {
    set_current_idx((prevIndex) => (prevIndex === 0 ? ReduxData.length - 1 : prevIndex - 1))
  }

  // Function to handle the right arrow click
  const handleRightClick = async () => {
    set_current_idx((prevIndex) => (prevIndex === ReduxData.length - 1 ? 0 : prevIndex + 1))
  }
  useEffect(()=>{
    getFeedData()
  },[])
  return (
    <div className='flex justify-center items-center'>
      <FaArrowCircleLeft onClick={handleLeftClick} className=' size-12 ' />
       {ReduxData && <UserCard  props = {ReduxData[current_idx]} old_index = {current_idx} index = {set_current_idx} />}
      <FaArrowCircleRight onClick={handleRightClick} className=' size-12 my-16'/>
    </div>
  )
}
